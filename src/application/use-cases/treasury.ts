...
export async function listMovements(
  ...
  const [movements, treasuries, reasons, team] = await Promise.all([
    // ⚠ كان هنا `limit: 50` — اتشال.
    //
    // الحركة رقم ٥١ كانت بتختفي من الشاشة بلا أي علامة. الموظّف
    // يدوّر على حركة سجّلها امبارح ومايلاقيهاش، ويفتكر إنها
    // اتمسحت.
    //
    // ⚠ التمن مكتوب عشان يكون معروف: الشاشة دي دلوقتي بتبني كل
    // حركات الفرع في صفحة واحدة. المحل اللي بيسجّل ٥٠ حركة في
    // اليوم بيوصل ١٨ ألف حركة في السنة — والصفحة هتتقل مع الوقت
    // على سفاري.
    //
    // البديل لو حصل: كشف الحساب المرحَّل (`/treasury/<id>`) موجود
    // وبيترحّل بمؤشّر، فمفيش حاجة بتضيع فيه أصلاً.
    deps.movements.list({ tenantId: actor.tenantId, branchId: branchScope, status }),
...
/**
 * حجم الصفحة الافتراضي والأقصى.
 *
 * ══ ⚠ ده مش سقف على البيانات ══
 * الرقمين دول بيحكموا **حجم الطلبة الواحدة** بس. الكشف بيكمّل
 * من المؤشّر لحد آخر حركة في تاريخ الخزنة، فمفيش صف بيضيع.
 *
 * وعشان كده فضلوا مكانهم لما شِلنا باقي السقوف: من غيرهم، أي حد
 * يقدر يبعت `?limit=999999` في الرابط ويطلب من السيرفر يبني
 * الدنيا كلها في رد واحد.
 *
 * الفرق: السقف الصامت بياخد منك بيانات وماتعرفش. ده بياخد منك
 * **سرعة** بس، والباقي جاي في الصفحة اللي بعدها.
 */
